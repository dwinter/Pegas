...
class.\footnote{For details:
  ape.mpl.ird.fr/pegas/DefinitionDataClassesPegas.pdf.}
...
brief
account of the data; the option \code{details = TRUE} allows to print
them like a standard data frame. If the same data were formatted with
a different allele separator (file `titi'):
...
args(read.loci)
@ 
We already know \code{file}, \code{header}, and
\code{allele.sep}. \code{loci.sep} is the separator of the columns
(not only the loci) which is one or several spaces by default (use
\code{"\textbackslash t"} if a tabulation). \code{col.pop} must an
...
\adegenet\ includes four data files in each of these format of the
